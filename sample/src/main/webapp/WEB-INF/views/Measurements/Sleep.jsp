...
<body>
        ...
        <div class="row">
            <div class="col-lg-12">
	            <c:if test="${pageContext.request.userPrincipal.name == 'doctor'}"> 
                	<h1><i class="glyphicon glyphicon-bed"></i> Sleep Measurements</h1>
					<h3>Patient: ${patient_name}</h3>
					<hr />
				</c:if>
	            <c:if test="${pageContext.request.userPrincipal.name != 'doctor'}"> 
	            	<h1 class="page-header"><i class="glyphicon glyphicon-bed"></i> Sleep Measurements</h1>
	            </c:if>
            </div>
            <!-- /.col-lg-12 -->
        </div>
		                            ...
		                            <table id="activity_table" class="table table-striped table-bordered table-hover display">
		                                <thead>
											<tr>
												...
												<th>Awake (min)</th>
												<th>Efficiency (%)</th>
												<th>Fall Asleep (min)</th>
												<th>After Wakeup (min)</th>
											</tr>
										</thead>
									</table>
								</div>
	...
	$(document).ready(function() {
		...
		$('#activity_table').DataTable( {
	        ajax: {
	            url: "get/sleep?tokenID=" + location.search.split('tokenID=')[1]
	        },
	        columns: [
	            { data: "source" },
	            { data: "date" },
	            ...
	            { data: "timeAwake" },
	            { data: "efficiency" },
	            { data: "timeToFallAsleep" },
	            { data: "timeAfterWakeup" }
	        ],
	        order: [ 1, 'desc' ],
	        pageLength: 10
	    });
	...
	function renderLiveTempGraph() {
		  ...
		  // Call our API
		  $.getJSON('get/sleep?tokenID=' + location.search.split('tokenID=')[1],
		    function(results) {
