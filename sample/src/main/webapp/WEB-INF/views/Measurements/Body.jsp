...
<head>
	...
	<title>BMI/Body Fat Measurements</title>
...
<body>
        ...
        <div class="row">
            <div class="col-lg-12">
	            <c:if test="${pageContext.request.userPrincipal.name == 'doctor'}"> 
                	<h1><i class="glyphicon glyphicon-user"></i> BMI / Body Fat Measurements</h1>
					<h3>Patient: ${patient_name}</h3>
					<hr />
				</c:if>
	            <c:if test="${pageContext.request.userPrincipal.name != 'doctor'}"> 
	            	<h1 class="page-header"><i class="glyphicon glyphicon-user"></i> BMI / Body Fat Measurements</h1>
	            </c:if>
            </div>
            <!-- /.col-lg-12 -->
        </div>
		    	                ...
		    	                <div class="dataTable_wrapper">
									<div class="btn-group" data-toggle="buttons">
										<label id="button1" class="btn btn-primary active">
											<input type="radio" name="options" id="option1" autocomplete="off" checked>BMI
										</label>
										<label id="button2" class="btn btn-primary">
											<input type="radio" name="options" id="option2" autocomplete="off">Body Fat
										</label>
									</div>
	...
	$(document).ready(function() {
		...
		$('#activity_table').DataTable( {
	        ajax: {
	            url: "get/body?tokenID=" + location.search.split('tokenID=')[1]
	        },
	        columns: [
	            { data: "source" },
	            { data: "date" },
		...
		bmiData = $.getJSON('get/bmi?tokenID=' + token, function(result){
			drawGraph(result, 'BMI', 'green', ' kg/m2');}),
		bodyFatData = $.getJSON('get/body_fat?tokenID=' + token);

		$("#button1").click(function() {drawGraph(bmiData.responseJSON, 'BMI', 'green', ' kg/m2');});
		$("#button2").click(function() {drawGraph(bodyFatData.responseJSON, 'Body Fat', 'blue', '%');});
